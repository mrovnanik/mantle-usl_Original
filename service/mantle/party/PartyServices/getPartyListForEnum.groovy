...
EntityFind ef = ec.entity.find("mantle.party.FindPartyViewEnhanced").distinct(true)
...
if (showOnlyEnabled == true) {
    cndDisabledParty = [
            ec.entity.conditionFactory.makeCondition("disabled", EntityCondition.EQUALS, "N"),
            ec.entity.conditionFactory.makeCondition("disabled", EntityCondition.EQUALS, NULL)]
    ef.condition(ec.entity.conditionFactory.makeCondition(cndDisabledParty, EntityCondition.OR))
}
...
//ec.logger.info("idValue = ${idValue} ${idValue.getClass()}")

if (idValue) {
    if (idValue.contains(':')) {
        def ptidIco = ['ico','ičo','ič','i','id']
        def ptidDic = ['tax','dic','dič','d','t']

        switch (idValue.tokenize(':')[0]) {
            case {  ptidIco.contains(it) }:
                ef.condition(ec.entity.conditionFactory.makeCondition("idValueIco", EntityCondition.LIKE, (leadingWildcard ? "%" : "") + idValue.tokenize(':')[1] + "%").ignoreCase())
                break
            case { ptidDic.contains(it) }:
                ef.condition(ec.entity.conditionFactory.makeCondition("idValueDic", EntityCondition.LIKE, (leadingWildcard ? "%" : "") + idValue.tokenize(':')[1] + "%").ignoreCase())
                break
        }
    }
}
...
//ec.logger.warn("======= get#PartyListForEnum cond: ${ef.getWhereEntityCondition()}")
//ec.logger.warn("======= ef.count(): ${ef.count()}")
